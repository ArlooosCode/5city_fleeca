            ...
            while true do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "[~r~E~w~] Zbierać Pieniądze", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "[~r~E~w~] Zbierać Pieniądze", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "[~r~E~w~] Zbierać Pieniądze", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" then
        exports["mythic_notify"]:SendAlert("inform", "Uruchomią się alarmy banku!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
            ...
            if PlayerData.job.name == "police" and not dooruse then
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 4.0 then
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Odblokuj drzwi", 0.40)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Zamknij drzwi", 0.40)
                            end
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    exports['progressBars']:startUI(2000, "Korzystanie z Karty Bezpieczeństwa")
    Citizen.Wait(1500)
    ...
    exports["mythic_notify"]:SendAlert("success", "Hakowanie zakończone!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
    TriggerServerEvent("utk_fh:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    exports["mythic_notify"]:SendAlert("error", "Masz 2 minuty do aktywacji systemu bezpieczeństwa.")
...
function SecondDoor(data, key)
    ...
    exports["mythic_notify"]:SendAlert("success", "Dostęp Przyznany!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
                ...
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    ESX.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            exports["mythic_notify"]:SendAlert("error", "Nie masz Karty Bezpieczeństwa.")
                        end
                    end)
                end
            end
