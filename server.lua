        ...
        if item >= 1 then
            if not UTK.Banks[bank].onaction == true then
                if (os.time() - UTK.cooldown) > UTK.Banks[bank].lastrobbed then
                    ...
                    TriggerClientEvent("utk_fh:outcome", _source, false, "Ten bank niedawno Został obrabował. Musisz poczekać "..math.floor((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)) / 60)..":"..math.fmod((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)), 60))
                end
            else
                TriggerClientEvent("utk_fh:outcome", _source, false, "Ten bank jest obecnie rabowany.")
            end
        else
            TriggerClientEvent("utk_fh:outcome", _source, false, "Nie masz złośliwej Karty Dostępu")
        end
    else
        TriggerClientEvent("utk_fh:outcome", _source, false, "Brak Wystarczającej Ilośći LSPD.")
    end
end)
